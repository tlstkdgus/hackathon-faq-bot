...
- `ALLOWED_CHANNEL_IDS`로 학생용 명령 3개의 사용 채널을 제한한다.
  비우면 제한 없음(기존 동작). 허용 밖에서는 **응답 자체를 안 한다** —
  디스코드가 본인에게만 "응답하지 않았습니다"를 띄우고 채널엔 아무것도 안 남는다.
  운영 결정이라 안내 메시지로 바꾸지 말 것. 대신 **공지에 사용 채널을 반드시 명시**해야 한다
  (학생 눈엔 봇 고장으로 보인다). 운영진 명령(`/해커톤통계`·`!리로드`)에는 적용하지 않는다
